  ...
  actions: {
    ...
    async requestSessionFromServer({ getters, commit }) {
      ...
      return new Promise((resolve,reject) => {
        ...
        console.log(
          'requestSessionFromServer(): request=' + JSON.stringify(request)
        )
        try {
      ...
      Utils.getIpAddressForClient()
        .then(ip => {
          ...
          if (getters['session/getSessionId'] === null) {
            console.log(
              'updateIpAddress(): Getting SessionId for IP Address:' + ip
            )
    ...
    async fetchExistingSessionFromServer({ getters, commit }) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (sessionId) {
          ...
          console.log(
            'fetchExistingSessionFromServer(): request=' +
              JSON.stringify(request)
          )

          this._vm.$socket.emit(SOCKET_EVENTS.CHECK_SESSION, request, resp => {
            console.log(
              'fetchExistingSessionFromServer():' + JSON.stringify(resp)
            )
    ...
    sendRemoveSessionRequest({ getters, commit }) {
      new Promise((resolve) => {
        ...
        console.log(
          'sendRemoveSessionRequest(): request' + JSON.stringify(request)
        )
