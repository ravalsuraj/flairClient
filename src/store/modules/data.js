      ...
      return sugarconnector.fetchOuathTokenForSugarCrm().then(resp => {
        if (resp && resp.data && resp.data.access_token) {
          ...
          dispatch("showErrorBanner", [
            "CRM error...",
            "Sugar CRM authentication failure. Cannot retreive CRM data for screenpop"
          ]);
        }
      });
    },
        ...
        sugarconnector
          ...
          .then(resp => {
            if (resp.data.records.length > 0) {
              ...
              dispatch("showErrorBanner", [
                "CRM Screenpop failed",
                "We were unable to fetch a record for the number " + phoneNumber
              ]);
              reject(resp);
            }
          })
